...
public class CompNamingContext implements Context 
{
    ...
    private static Hashtable binders = new Hashtable();
    
    
    private Hashtable myEnv = null;
    private Hashtable bindings = new Hashtable();
    static private NameParser myParser = new GeneralNameParser();
    ...
    public CompNamingContext() throws NamingException 
    {
        myEnv = new Hashtable();
        compId = "server";
        setBinds(compId,myEnv);
    ...
    public CompNamingContext(Hashtable env)
    {
        myEnv = env;
        compId = "server";
    ...
    private void setBinds(String id, Hashtable env)
    {
        
        String key = id+env.toString();
        
        try
        {
            Object obj = binders.get(key);
            
            if(obj != null)
            {
                bindings = (Hashtable) obj;
                return;
            }
                
        }
        catch(Exception e) {
            System.err.println("ERROR CompNamingContext"+e.toString());
            }
        
        binders.put(key, bindings);
    }
	...
	if (n.size() == 1) {
		...
		InitialContext ictx = NamingManager.getInstance().getInitialContext();
	...
	if (obj instanceof Context) {
	    ...
	    //Trace.errln("CompNamingContext: can only list a Context");
	...
	//Trace.errln("CompNamingContext try to destroySubcontext "+name);
	...
	Name n = new CompositeName(name);
	if (n.size() < 1) {
	    //Trace.errln("CompNamingContext createSubcontext with empty name ?");
	...
	//Trace.errln("CompNamingContext lookupLink not implemented yet!");
	return lookup(name);
    }
	...
	//Trace.errln("CompNamingContext composeName not implemented!");
	...
	//Trace.errln("CompNamingContext composeName "+name+" "+prefix);
