...
public class Base64
{
    ...
    static public byte[] encodeBytes(byte[] data)
    {
        ...
        for (int i = 0, index = 0; i < data.length; i += 3, index += 4)
        {
            ...
            out[index + 3] = alphabetBytes[(quad ? (val & 0x3F) : 64)];
            val >>= 6;
            out[index + 2] = alphabetBytes[(trip ? (val & 0x3F) : 64)];
            val >>= 6;
            out[index + 1] = alphabetBytes[val & 0x3F];
            val >>= 6;
            out[index + 0] = alphabetBytes[val & 0x3F];
        }
        return out;
    }
    ...
    static private char[] alphabet = ALPHA.toCharArray();

    static private byte[] alphabetBytes = ALPHA.getBytes();
