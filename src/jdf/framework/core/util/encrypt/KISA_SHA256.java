...
public class KISA_SHA256 {
	...
	*/
	...
	public static void main(String[] args) throws Exception
	{
		...
		String plainText = "_0000_DEV_TPTKDDPTJKWKDQKQH_";
		int plainTextLen = plainText.length();
		byte[] encryptKeyBytes = new byte[32];
		
		SHA256_Encrpyt( plainText.getBytes(), plainTextLen, encryptKeyBytes );
		System.out.println("암호화키 : "+CipherUtil.hexToString(encryptKeyBytes));
		
		byte[] encrypt = CipherUtil.encode(encryptKeyBytes, "1111".getBytes("utf-8"));
		
		System.out.println("암호화 : "+CipherUtil.hexToString(encrypt));
		
		System.out.println("복호화 : "+new String(CipherUtil.decode(encryptKeyBytes, encrypt)));
