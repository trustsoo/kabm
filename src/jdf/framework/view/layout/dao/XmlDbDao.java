 ...
 */
public class XmlDbDao implements LayoutDao
{
    private final static String LOG_ID = "<t:XmlDbDao> ";
    ...
    public void load(Map layoutStore, List layoutStoreList) throws Exception
    {
        ...
        try
        {
            ...
            output = interact.execute("/anytemplet/getLayout", new DataSet());
        ...
        String layoutXmlStr = output.getText("layout");

        ByteArrayInputStream bis = new ByteArrayInputStream(layoutXmlStr.getBytes());
        Document docu = DocBuilder.getDocument(bis);

        XMLReferer doc = new XMLReferer(docu);
