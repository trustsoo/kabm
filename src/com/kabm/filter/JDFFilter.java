 ...
 * @author 김성권
 */
public class JDFFilter implements Filter, MenuContext, JDFContext {
    private final static String LOG_ID = "<t:UniversalFilter> ";
    ...
    private String acl_dispath_page;
    ...
    public void init(FilterConfig filterConf) throws ServletException {
        ...
        try {
            ...
            acl_dispath_page = getParameter(filterConf, "acl-page");
            if (acl_dispath_page != null && acl_dispath_page.length() == 0)
            	acl_dispath_page = null;
    ...
    public void doFilter(ServletRequest request, ServletResponse response,
        ...
        /*if(!uri.startsWith("www.") && !uri.startsWith("m.") && !uri.startsWith(thisServerIP) && !uri.startsWith(thisPublicIP))
        {
            String[] tempArray = SmartStringArray.split("/", uri);          
            String[] domainSep = SmartStringArray.split(".", tempArray[0]);
            
            if(domainSep.length >= 3)
            {
                int kkk = 0;
                for(String temp : domainSep)
                {
                    if(temp.length() > 1 && thisServerDomain.indexOf(temp) != -1)
                    {
                        break;
                    }
                    tenant_id = tenant_id+temp+".";
                    kkk++;
                }
                
                tenant_id = tenant_id.substring(0, tenant_id.length() - 1);
                tenant_id = tenant_id.toLowerCase(); 
            }
            
            
            //Logger.debug.println(">>>>>>>>>>>>>>>>>>>>>>>>>>>>"+tenant_id);
            
            if(!"".equals(tenant_id))
            {
                request.setAttribute(MenuItem.MULTI_TENANT_KEY, tenant_id);
            } else
            {
        ...
        {           
            ...
            String rolename = "generalUser";
            AclInfo acl = AclInfo.getAclInfoInstance();
            String authLevel = menu.getAuthLevel()[0];
            if(authLevel != null && ( "A".equals(authLevel) ||  "S".equals(authLevel) ) )
            	rolename = "adminUser";
            RoleInfo role = acl.getRoleInfo(rolename);
            if (role == null)
                role = acl.getDefaultRoleInfo();
            this.login_page = role.getLoginUrl();
...
//System.out.println(respWrap.toString());
        ...
        {
            Logger.info.println(LOG_ID+"[REQ] "+jdf.framework.view.menu.util.RequestURL.getFullUrl(req)+" AJAX:"+isAjax+" "+" LANG:"+clientLanguage);
            
            if(!isAjax)         
            {               
            	dispatchAclPage(request, response, pe, tmplJsp);
            }
            else
            {
                dispathErrorXML(request, response, pe, "403");
            }
        }
    ...
    private boolean dispatchAclPage(ServletRequest request,
            ServletResponse response, Exception err, String templetPage) {
        
        //templetPage = null;
        
        if (this.acl_dispath_page == null || err == null)
            return false;
        ...
        try {
            ...
            if (templetPage != null)
            {
                ...
                RequestDispatcher rd = request.getRequestDispatcher(acl_dispath_page);
                ...
                System.out.println("* ------> "+acl_dispath_page);
                RequestDispatcher rd = request
                        .getRequestDispatcher(acl_dispath_page);
                rd.forward(request, response);
