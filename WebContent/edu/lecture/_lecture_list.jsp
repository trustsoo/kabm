...
function js_getOfflineEduinfo()
{
	var http = jQuery.ajax({
		url : '/edu/lecture/action/lectureAction.jspx?cmd=getOfflineEduInfo',
		data : 'brd_mng_no=8',
		type : 'POST',
		async : false,
		datatype: 'json',
		...
		success:function(json) 
		{
			var code = json.result.code;
	   		...
	   		if(code == '200')
	   		{
	   			if(data.offline_edu.length > 0 )
   				{
		   			jQuery('#ttl').html( unescapeHTML(json.result.data.offline_edu[0].ttl) );
		   			jQuery('#cntnt').html( unescapeHTML(json.result.data.offline_edu[0].cntnt) );
		   			
		   			if(json.result.data.offline_edu[0].is_exist_file == 'Y')
		   			{
		   				js_getFileEntries('gridFileEntriesDiv',false, jQuery('#attach_div_cd').val(), json.result.data.offline_edu[0].brd_no);
		   			}
   				}
			}
		}
...
<div id="content">
		<!-- start :: content -->
			...
			<div class="mypage">
				<div class="section-1">
					<ul class="applybtns" style="width: 915px;">
						<li style="width: 445px;height: 450px;">
							<p style="color:red;font-weight:bold;font-size:30px;margin:10px;padding:0;text-align:center;"></strong><br>
							<strong style="padding-top: 0px;">신규개설 및 승계자 교육</strong></p>
							<div style="text-align:left;display:block;padding-left: 30px;">
							<p style="height: 60px;">
							...
							<p style="height: 50px;">	
								<b>(<font style="color:red;font-weight:bold;font-size:20px;">"★반드시 대표자 본인"</font>이 직접 이수해야 함)<br>(교육생=대표자)</b>
							</p>
							<br>
							</div>
							<a href="javascript:fn_lectureView(1, '${new_lecture_seq_no}' , '${new_req}' , '${new_std_yyyy}' );" class="pbtn02"><span class="">교육신청</span></a>
						</li>
						<li style="width: 445px;height: 450px;">
							<p style="color:red;font-weight:bold;font-size:30px;margin:10px;padding:0;text-align:center;">${old_std_yyyy } 년도<br>
							<strong style="padding-top: 0px;">공중위생관리책임자 교육</strong></p>
							<div style="text-align:left;display:block;padding-left: 30px;">
							<p style="height: 70px;">
								<span style="font-size:14px;font-weight:bold;">수강신청기간</span> <br />
							...
							<a href="javascript:fn_lectureView(2, '${old_lecture_seq_no}' , '${old_req}' , '${old_std_yyyy}' );" class="pbtn02_1"><span class="">교육신청</span></a>
						</li>
					</ul>					
				</div>
			...
			<div class="view" style="margin-top:0px;">
				<div class="list_type2">
					...
					• 공중위생관리책임자(기존업체) : ${old_start } ~ 12월31일까지 수강가능 <br>
				...
				<table cellpadding="0" cellspacing="0" class="" summary="" >
					<tbody>
						<tr>
							...
							<td>
										<strong style="color:green">[스마트폰으로는 수강하실 수 없습니다.] </strong>
