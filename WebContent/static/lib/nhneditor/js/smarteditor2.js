...
nhn.husky.SE_PasteHandler = jindo.$Class({
	 ...
	 * */
	//IE8에서 오른쪽 버튼 클릭시 텍스트 사라지는 현상해결..edited by advan94(2014.11.12)
	...
	$ON_TABLE_QE_SET_IMG : function(elSelected){
		...
		if(n === "0"){
			...
			for(var j = 0, nLen = this.aSelectedCells.length; j < nLen ; j++){
				jindo.$Element(this.aSelectedCells[j]).css("backgroundImage", "url("+"http://static.se2.naver.com/static/com/img/"+sImageName+")");
				this.aSelectedCells[j].removeAttribute(this.TMP_BGC_ATTR);
				this.aSelectedCells[j].setAttribute(this.TMP_BGIMG_ATTR, "url("+"http://static.se2.naver.com/static/com/img/"+sImageName+")");
			}
		} 
